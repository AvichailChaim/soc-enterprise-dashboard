...
**איך עוצרים זמנית (לתחזוקה, בלי להסיר)** - גם זה דורש את אותה סיסמה: `.\control.ps1 -Action Stop -Password "הסיסמה" -MaintenanceMinutes 60` (פותח "חלון תחזוקה" בזמן שבו ה-Watchdog לא ינסה להחזיר את השירות ולא יתריע). לחזרה מוקדם יותר: `.\control.ps1 -Action Start -Password "הסיסמה"`. אחרי שהזמן שהוגדר חולף, ה-Watchdog מפעיל את השירות מחדש אוטומטית ממילא. **חשוב:** עצירה בכל דרך אחרת (Stop-Service, sc stop, nssm stop, Services.msc) תיחסם ע"י ההקשחה, ואם בכל זאת תצליח (למשל דרך הרשאות SYSTEM חיצוניות), ה-Watchdog יזהה זאת תוך עד 5 דקות, יפעיל את השירות מחדש וישלח התראת Critical לדשבורד.
...
## 8.2 ניהול מחשבים מרחוק דרך הדשבורד (עמוד Computers)

לחיצה על ⛭ בסרגל הצד פותחת עמוד "Computers" - רשימת כל המחשבים שדיווחו אי-פעם, עם כפתורי Stop / Start / Remove לכל מחשב. הפעולה מתבצעת דרך ה-Watchdog של אותו מחשב (בודק כל 5 דקות אם יש פקודה ממתינה) - כלומר יש עד כ-5 דקות עיכוב, וזה דורש שהמחשב מחובר לאינטרנט.

**חשוב:** גם דרך הדשבורד, כל פעולה דורשת את סיסמת ההסרה (`-UninstallPassword`) שהוגדרה על **אותו מחשב ספציפי** ב-`install.ps1`. גישה לדשבורד (משתמש+סיסמה) לבדה לא מספיקה - זו הגנה כפולה במכוון: גם מי שיש לו גישה לדשבורד לא יכול להסיר סוכנים בלי לדעת גם את הסיסמה המקומית של אותו מחשב.

מכיוון שה-Watchdog הוא זה שמבצע את הפקודות (ולא הסוכן הראשי עצמו), Stop/Start/Remove עובדים דרך הדשבורד גם אם השירות הראשי כבר עצור.
